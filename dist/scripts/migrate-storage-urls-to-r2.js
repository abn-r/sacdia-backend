...
function allowBareKeyForLiteralValue(value) {
    const normalized = value.trim();
    if (!normalized)
        return false;
...
function buildPublicUrl(publicBaseUrl, relativeKey) {
...
function extractSupabaseStoragePath(urlPath) {
    const match = urlPath.match(/(?:^|\/)storage\/v1\/object\/(?:public|sign)\/([^/]+)\/(.+)$/i);
    if (!match)
        return null;
    return {
        bucket: match[1],
...
function extractRelativePathFromConfiguredBase(urlPath, target) {
...
function deriveRelativeKey(candidate, target, allowBareKey) {
...
function migrateAssetUrlValue(value, target) {
    const raw = value.trim();
    if (!raw)
        return null;
    if (hasControlCharacters(raw))
        return null;
    if (isLikelyLocalFilePath(raw))
        return null;
    if (!looksLikeUrl(raw)) {
        const relative = deriveRelativeKey(raw, target, allowBareKeyForLiteralValue(raw));
        if (!relative)
            return null;
    ...
    for (const candidate of candidates) {
        ...
        if (seen.has(cacheKey))
            continue;
        seen.add(cacheKey);
        const relative = deriveRelativeKey(candidate.key, target, candidate.allowBareKey);
        if (!relative)
            continue;
        const next = buildPublicUrl(target.publicBaseUrl, relative);
        if (next !== raw) {
...
function addSample(result, sample, max = SAMPLE_LIMIT_PER_TABLE) {
    if (result.samples.length >= max)
        return;
    ...
    if (onlyRaw) {
        for (const token of onlyRaw.split(',')) {
            ...
            if (!ALL_TABLES.includes(table)) {
                throw new Error(`Invalid table in --only: ${token}. Allowed: ${ALL_TABLES.join(', ')}`);
            }
...
async function migrateUsers(prisma, options, targets) {
    const result = {
        table: 'users',
        scanned: 0,
        changed: 0,
    ...
    for (const row of rows) {
        ...
        if (!migrated || migrated === row.user_image)
            continue;
        result.changed += 1;
        addSample(result, {
            id: row.user_id,
        ...
        try {
            ...
            result.errors += 1;
            console.error(`[users:${row.user_id}] Failed to update user_image`, error);
        }
    }
    return result;
}
async function migrateActivities(prisma, options, targets) {
    const result = {
        table: 'activities',
        scanned: 0,
        changed: 0,
    ...
    for (const row of rows) {
        ...
        if (!migrated || migrated === row.image)
            continue;
        result.changed += 1;
        addSample(result, {
        ...
        try {
            ...
            result.errors += 1;
            console.error(`[activities:${row.activity_id}] Failed to update image`, error);
        }
    }
    return result;
}
async function migrateHonors(prisma, options, targets) {
    const result = {
        table: 'honors',
        scanned: 0,
        changed: 0,
    ...
    for (const row of rows) {
        result.scanned += 1;
        const nextImage = migrateAssetUrlValue(row.honor_image, targets.honorsImages);
        const nextMaterial = migrateAssetUrlValue(row.material_url, targets.honorsPdf);
        const data = {};
...
async function migrateClasses(prisma, options, targets) {
    const result = {
        table: 'classes',
        scanned: 0,
        changed: 0,
    ...
    for (const row of rows) {
        ...
        if (!row.material_url)
            continue;
        const migrated = migrateAssetUrlValue(row.material_url, targets.classesDocuments);
        if (!migrated || migrated === row.material_url)
            continue;
        result.changed += 1;
        addSample(result, {
        ...
        try {
            ...
            result.errors += 1;
            console.error(`[classes:${row.class_id}] Failed to update material_url`, error);
        }
    }
    return result;
}
async function migrateUsersHonors(prisma, options, targets) {
    const result = {
        table: 'users_honors',
        scanned: 0,
        changed: 0,
    ...
    for (const row of rows) {
        ...
        const nextDocument = typeof row.document === 'string'
        ...
        try {
            ...
            result.errors += 1;
            console.error(`[users_honors:${row.user_honor_id}] Failed to update`, error);
        }
    }
    return result;
}
function printResultSummary(results, options) {
    ...
    console.log(`Totals -> scanned: ${totals.scanned}, changed: ${totals.changed}, applied: ${totals.applied}, errors: ${totals.errors}`);
    for (const result of results) {
        if (result.samples.length === 0)
            continue;
        console.log(`\nSamples for ${result.table}:`);
        for (const sample of result.samples) {
            console.log(`- [${sample.id}] ${sample.field}\n  before: ${sample.before}\n  after : ${sample.after}`);
        }
    }
}
async function main() {
    ...
    const results = [];
    console.log(`Starting storage URL migration in ${options.apply ? 'APPLY' : 'DRY RUN'} mode...`);
    try {
...
//# sourceMappingURL=migrate-storage-urls-to-r2.js.map