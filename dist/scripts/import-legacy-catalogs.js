...
function parseInsertSql(sql, sourceFile) {
    const cleanSql = sql.trim();
    const match = cleanSql.match(/^INSERT INTO\s+"public"\."([^"]+)"\s+\(([\s\S]*?)\)\s+VALUES\s+([\s\S]*);\s*$/i);
    if (!match) {
...
function splitTuples(rawValuesSection) {
    const tuples = [];
    let inString = false;
    let depth = 0;
    let current = '';
...
function applyTransform(sourceFile, sourceTable, columns, rows) {
...
function remapColumnValues(insert, columnName, idMap) {
...
async function findIdByColumn(pool, table, idColumn, lookupColumn, value) {
    if (value === null)
        return null;
...
async function buildCountryIdMap(pool, insert) {
    ...
    for (const row of insert.rows) {
        ...
        if (!legacyKey)
            continue;
        const byName = await findIdByColumn(pool, 'countries', 'country_id', 'name', row[nameIndex]);
        const resolvedId = byName ??
            (await findIdByColumn(pool, 'countries', 'country_id', 'abbreviation', row[abbreviationIndex]));
        if (resolvedId !== null) {
...
async function buildUnionIdMap(pool, insert) {
    ...
    for (const row of insert.rows) {
        ...
        if (!legacyKey)
            continue;
        const byAbbreviation = await findIdByColumn(pool, 'unions', 'union_id', 'abbreviation', row[abbreviationIndex]);
        const resolvedId = byAbbreviation ??
...
async function buildLocalFieldIdMap(pool, insert) {
    ...
    for (const row of insert.rows) {
        ...
        if (!legacyKey)
            continue;
        const byAbbreviation = await findIdByColumn(pool, 'local_fields', 'local_field_id', 'abbreviation', row[abbreviationIndex]);
        const resolvedId = byAbbreviation ??
...
async function buildDistrictIdMap(pool, insert) {
  ...
  `;
    for (const row of insert.rows) {
        ...
        if (!Number.isFinite(localFieldId))
            continue;
        const result = await pool.query(sql, [row[nameIndex], localFieldId]);
        if (result.rows.length > 0) {
...
function applyForeignKeyRemaps(insert, maps) {
...
async function refreshLegacyMapForTable(pool, insert, maps) {
...
async function ensureHonorsCategories(pool, insert) {
    const index = getColumnIndex(insert.columns, 'honors_category_id');
    if (index === -1)
        return;
    const ids = Array.from(new Set(insert.rows
        .map((row) => Number(row[index]))
        .filter((value) => Number.isInteger(value) && value > 0)));
    if (ids.length === 0)
        return;
    const existingResult = await pool.query('SELECT "honor_category_id" FROM "public"."honors_categories" WHERE "honor_category_id" = ANY($1::int[])', [ids]);
...
async function syncSequence(pool, table, idColumn) {
    const serialQuery = `
    SELECT pg_get_serial_sequence($1, $2) AS sequence_name
  `;
    const serialResult = await pool.query(serialQuery, [
        `public.${table}`,
        idColumn,
    ]);
...
async function loadParsedInserts(sourceDir) {
    const inserts = [];
    for (const fileName of FILE_ORDER) {
        if (EXCLUDED_FILES.has(fileName)) {
            continue;
        }
        const fullPath = node_path_1.default.join(sourceDir, fileName);
        const rawSql = await promises_1.default.readFile(fullPath, 'utf-8');
...
async function run() {
    ...
    for (const result of results) {
        if (dryRun) {
            console.log(`${result.file}: ${result.parsedRows} rows parsed (${result.sourceTable} -> ${result.targetTable})`);
            continue;
        }
        const status = result.failedRows === 0
            ...
            : result.insertedRows === 0
                ? 'failed'
                : 'partial';
        console.log(`${result.file}: ${status}; parsed=${result.parsedRows}, inserted=${result.insertedRows}, failed=${result.failedRows}`);
...
//# sourceMappingURL=import-legacy-catalogs.js.map