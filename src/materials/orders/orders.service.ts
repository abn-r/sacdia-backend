...
const ORDER_INCLUDE = {
  lines: {
    include: {
      ...
      variant_option: { select: { id: true, label: true } },
    },
  },
  creator: {
    select: {
  ...
  local_field: {
    select: { local_field_id: true, name: true, abbreviation: true },
  },
...
export class OrdersService {
  ...
  async createOrder(dto: CreateOrderDto, userId: string): Promise<OrderDto> {
    ...
    // Derive the local_field this order belongs to from the director's
    // club_section → club. The director's club determines the LF; we never
    // trust a client-supplied local_field_id.
    ...
    if (!section || !section.clubs) {
      throw new NotFoundException({
        code: 'club_section_not_found',
        message: `Club section ${dto.club_section_id} not found`,
      });
    }
    ...
    // Scope to the order's local_field so directors can't order products
    // that belong to a different LF catalog.
    const products = await this.prisma.materialProduct.findMany({
      where: { id: { in: productIds }, local_field_id: localFieldId },
      select: {
        id: true,
        price_centavos: true,
        active: true,
    ...
    // Read envio_centavos_default from the LF's material_config (per-LF row).
    // Missing row → fallback to 0 (we don't fail order creation just because
    // the campo hasn't filled in their delivery config yet).
    ...
    // Atomic insert: allocate folio + create order + lines in one transaction.
    // Folio is assigned at creation (not at approval) so directors can quote
    // a tracking reference from the moment the order is placed. Cancelled
    // en_revision orders therefore burn folios — accepted trade-off.
    const order = await this.prisma.$transaction(async (tx) => {
      const { folio_referencia } = await this.folioService.allocate(
        tx,
        localFieldId,
      );
  ...
  // PATCH /materials/orders/:folio/lines/:lineId
  // -------------------------------------------------------------------------
  ...
  async list(
    ...
    const data: OrderSummaryDto[] = rows.map((r) => ({
      id: r.id,
   ...
   * 404 if not found.
   */
  async getByFolio(folio: string, actor: ActorContext): Promise<OrderDto> {
    const order = await this.prisma.materialOrder.findFirst({
      where: this.orderIdentifierWhere(folio),
      include: { ...ORDER_INCLUDE },
    });
   ...
   */
  async getById(id: string, actor: ActorContext): Promise<OrderDto> {
  ...
  async historial(
    ...
    // (which already matches every order they themselves created).
    return this.list(query, { id: userId, canApprove: false }, localFieldId);
  }

  // -------------------------------------------------------------------------
  // T3b.4 — approve  (REQ-ORD-007, REQ-ORD-008, REQ-INV-004, REQ-CFG-003,
  //                    SC-03, SC-04, SC-07, SC-18)
  // POST /materials/orders/:folio/approve
  // -------------------------------------------------------------------------

  async approve(folioOrId: string, actor: { id: string }): Promise<OrderDto> {
    // Load order + lines
    const order = await this.prisma.materialOrder.findFirst({
    ...
    // The snapshot comes from the order's local_field (per-LF config).
    const config = await this.prisma.materialConfig.findUnique({
    ...
    // Atomic transaction: lock products + decrement stock + snapshot bank +
    // mark approved. Folio is NOT (re-)allocated here — it was assigned at
    // creation time so the director can quote it from the moment they place
    // the order.
    const updatedOrder = await this.prisma.$transaction(async (tx) => {
      ...
      // Persist state transition + bank snapshot + totals (folio stays)
      const updated = await tx.materialOrder.update({
        where: { id: order.id },
        data: {
          estado: 'aprobada',
  ...
  // POST /materials/orders/:folio/cancel
  // -------------------------------------------------------------------------
  ...
  // POST /materials/orders/:folio/deliver
  // -------------------------------------------------------------------------

  async deliver(folioOrId: string, actor: { id: string }): Promise<OrderDto> {
  ...
  private mapOrder(order: {
    ...
    return {
      ...
      lines: order.lines.map((l) => ({
        ...
        product: l.product
          ? { id: l.product.id, sku: l.product.sku, title: l.product.title }
          : null,
        variant_option: l.variant_option
          ? { id: l.variant_option.id, label: l.variant_option.label }
          : null,
      })),
      ...
      director: {
        nombre:
          [
            order.creator.name,
            order.creator.paternal_last_name,
            order.creator.maternal_last_name,
          ]
            .filter(Boolean)
            .join(' ')
            .trim() || null,
      ...
      local_field: {
        local_field_id: order.local_field.local_field_id,
        name: order.local_field.name,
        abbreviation: order.local_field.abbreviation,
      },
    };
  }
}
