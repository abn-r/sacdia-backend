...
/**
 * Comprobante file validation pipe.
 *
 * Validates:
...
export const COMPROBANTE_ALLOWED_MIMES = [
  'application/pdf',
  'image/jpeg',
  'image/png',
] as const;
 ...
 */
export function extensionFromMime(mime: ComprobanteMime): string {
  const map: Record<ComprobanteMime, string> = {
    'application/pdf': 'pdf',
    'image/jpeg': 'jpg',
    'image/png': 'png',
  };
...
export class ComprobanteFileValidationPipe implements PipeTransform {
  transform(file: Express.Multer.File | undefined): Express.Multer.File {
    ...
    const allowedSet: readonly string[] = COMPROBANTE_ALLOWED_MIMES;
    if (!allowedSet.includes(file.mimetype)) {
      throw new BadRequestException({
        code: 'invalid_file',
        reason: 'unsupported_mime',
        message: `MIME type '${file.mimetype}' is not allowed. Accepted: ${COMPROBANTE_ALLOWED_MIMES.join(', ')}`,
      });
    }

    // Magic-byte check — prevents MIME spoofing
    const mime = file.mimetype as ComprobanteMime;
