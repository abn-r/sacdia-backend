...
const createLegacySchema = (client: Client, schema: string) =>
  client.query(`CREATE SCHEMA ${schema}; SET search_path=${schema},public;
    CREATE TABLE users (user_id UUID PRIMARY KEY); CREATE TABLE club_sections (club_section_id INT PRIMARY KEY); CREATE TABLE finances_categories (finance_category_id INT PRIMARY KEY, type INT NOT NULL); CREATE TABLE finances (finance_id INT PRIMARY KEY);
    INSERT INTO users VALUES ('00000000-0000-0000-0000-000000000001'), ('00000000-0000-0000-0000-000000000002'); INSERT INTO club_sections VALUES (7); INSERT INTO finances_categories VALUES (11, 0);
...
describe('finance ledger v2 foundation migration', () => {
  ...
  dbIt('enforces currencies, decisions and append-only events', async () => {
    ...
    try {
      ...
      await expect(
        client.query(
          `INSERT INTO finance_ledger_events (finance_ledger_entry_id, event_type, payload)
           VALUES ($1, 'CREATED_WITHOUT_ACTOR', '{}')`,
          [entry.rows[0].finance_ledger_entry_id],
        ),
      ).rejects.toMatchObject({ code: '23502', column: 'actor_user_id' });
      await expect(
        client.query(
