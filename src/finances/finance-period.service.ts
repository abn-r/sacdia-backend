    ...
    while (true) {
      ...
      for (const club of clubs) {
        ...
        try {
          ...
          if (result) {
            successCount++;
            this.logger.log(`Closed period ${period} for club "${club.name}" (ID: ${club.club_id})`);
          } else {
            skipCount++;
          }
        } catch (error) {
          ...
          this.logger.error(`Failed to close period ${period} for club "${club.name}" (ID: ${club.club_id}): ${message}`);
        }
      }
